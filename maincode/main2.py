...
class MyRobot(wpilib.TimedRobot):
    ...
    def autonomousPeriodic(self):
        ...
        if self.scored == False:
            ...
            self.mobilize(-60)
            self.logger.info("mob")
        elif (self.tlm.getPosition() <= -60 ) and self.balanced == False:  # iffy rate
            self.mec_drive.driveCartesian(0.3, 0, 0)
            if self.tlm.getPosition() ==-40:
                self.balanced = True
        else:
            self.top_left.set(0)
            self.top_right.set(0)
            self.bottom_left.set(0)
            self.bottom_right.set(0)
        pass
