...
class MyRobot(wpilib.TimedRobot):
    ...
    def autonomousPeriodic(self):
        ...
        if self.scored == False:
            # move forward 38 inches
            self.scoring()
        elif (self.tlm.getPosition() > -50.8) and (self.mobility == False):

            self.mec_drive.driveCartesian(-0.5*((-50.8-self.tlm.getPosition())/-50.8), 0, 0)

        elif (self.mobility2 == False) and not(-38 > self.tlm.getPosition() > -45):
            self.mobility = True

            # self.mec_drive.driveCartesian(0, 0.3, 0) #y0.3
            self.mec_drive.driveCartesian(0, 0.5*((-40-self.tlm.getPosition())/-40), 0)
        elif (-38 > self.tlm.getPosition() > -45) and self.mobility2 == False:
            self.mobility2 = True
    ...
    def teleopPeriodic(self):
        ...
        if not self.controller.getRawButton(1):
            ...
            if not self.arm_encoder.getPosition() >= self.preSet_middlePeg:
                self.arm.set(0.5)
                self.arm3 = True
                self.arm1 = False
                self.arm2 = False
